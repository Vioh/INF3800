...
public class Sieve<D, R extends Comparable<R>> implements Iterable<HeapItem<D, R>> {
	...
	public void sift(D data, R rank) {
		...
		// If the current item is better than the worst of the items seen so
		// far, forget about the worst item and keep the current item instead.
    ...
    /**
     * Implements the {@link Iterable<HeapItem<D, R>>} interface. Note that
     * the iterator does not iterate over the items in sorted order.
     */
